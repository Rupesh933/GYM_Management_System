...
class User(AbstractUser):  # username, email, password, first_name, last_name, are inherited from AbstratUser
    ...
    )  # To determine whether the currently logged-in user is an Admin user or a Member user.
...
class Payment(models.Model):
    ...
    payment_mode = models.CharField(
        max_length=50, choices=PAYMENT_MODE_CHOICES, null=True, blank=True
    )
...
class Enquiry(models.Model):
    ...
    status = models.CharField(
        max_length=50, choices=ENQUIRY_STATUS_CHOICES, default="NEW"
    )
...
class WorkoutPlan(models.Model):
    member = models.ForeignKey(
        MemberProfile,
        on_delete=models.CASCADE,  # if member is deleted, delete workout plan
    ...
    creation_at = models.DateTimeField(
        auto_now_add=True
    )  # TimeStamp when the workout plan
